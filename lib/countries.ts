...
const flag = (code: string) => code === 'XK'
  ? '🇽🇰'
...
export function getCountryOptions(locale: Locale): CountryOption[] {
  ...
  const localized = new Intl.DisplayNames([locale], { type: 'region' })
  const english = new Intl.DisplayNames(['en'], { type: 'region' })
  const options = ISO_CODES.map((code) => ({
    code,
    name: localized.of(code) ?? code,
    englishName: english.of(code) ?? code,
    flag: flag(code),
...
export function countryName(value: string | null, locale: Locale) {
  if (!value) return null
  const options = getCountryOptions(locale)
  const normalized = value.trim().toLowerCase()
  return options.find((country) =>
